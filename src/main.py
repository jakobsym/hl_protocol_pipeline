...
async def main():
    ...
    logger.info(f"Sys time: {datetime.datetime.now()}")
    logger.info(f"UTC time: {datetime.datetime.now(datetime.timezone.utc)}")
    ...
    token_blob_data = await blob_extractor.unpack_blob_data(blob=blob)
    ...
    for protocol in hyperliquid_dexs:
        try:
            ...
            # store transformed data into a dictionary payload
